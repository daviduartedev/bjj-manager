...
**SPEC-2.1.** Autenticação do professor (**login** e-mail/senha) e **acesso** à conta da academia após **provisionamento manual** do utilizador e do vínculo conta/perfil (ver **AUTH-**, **SPEC-3.7**, [`docs/security/rls.md`](../security/rls.md)). A área operacional autenticada tem entrada canónica em **`/painel`** (rotas em pt-BR; ver **SHELL-2**).
...
**SPEC-10.3.** O produto suporta **tema claro e escuro** com paridade legível; detalhes de componentes e exemplos de referência estão em **DS-** e em [`docs/design/style-guide.md`](../design/style-guide.md).
...
**SPEC-9.1.** Este arquivo e os demais em [`spec/product/`](../../spec/product/) são **canônicos**; cópias em [`docs/product/`](../product/) devem permanecer alinhadas.
